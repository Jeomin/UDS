...
def setup_agent(env, args):
    ...
    agent_params = {
        ...
        'action_dim': len(env.config['action_assets']),  # PPO直接输出每个泵的控制
        'actornet_layer_A': 3,
    ...
    if args.model_path:
        try:
            ...
            actor_path = os.path.join(model_dir, "PPOactor.h5")
            critic_path = os.path.join(model_dir, "PPOcritic.h5")
...
def collect_data_for_surrogate(agent, env, rainfall_data, num_samples=10):
    ...
    for rain in rainfall_samples:
        ...
        while not done:
            ...
            # 保存数据 - 对于PPO，动作是直接的泵控制状态
            dataset.append((s, action[0], logits, flooding, cso))
...
def main():
    ...
    output_dir = os.path.join(
        args.output_dir,
        f"interpretable_ppo_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    # 保存模型和可解释性分析结果
    print("保存结果...")
    agent.actor.save_weights(os.path.join(output_dir, 'ppo_actor.h5'))
    agent.critic.save_weights(os.path.join(output_dir, 'ppo_critic.h5'))
